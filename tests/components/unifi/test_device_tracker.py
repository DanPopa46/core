...
async def test_tracked_wireless_clients(hass, aioclient_mock,
    ...
    event = {
        ...
        "msg":
        f'User{[client["mac"]]} disconnected from "{client["essid"]}" (7m 47s connected, 448.28K bytes, last AP[{client["ap_mac"]}])',
    ...
    event = {
        ...
        "msg":
        f'User{[client["mac"]]} has connected to AP[{client["ap_mac"]}] with SSID "{client["essid"]}" on "channel 44(na)"',
...
async def test_tracked_devices(hass, aioclient_mock, mock_unifi_websocket):
    ...
    assert hass.states.get(
        "device_tracker.device_1").state == STATE_UNAVAILABLE
    assert hass.states.get("device_tracker.device_2").state == STATE_HOME
    ...
    event = {
        ...
        "key": "EVT_SW_Upgraded",
        "msg":
        f'Switch[{device_2["mac"]}] was upgraded from "{device_2["version"]}" to "4.3.13.11253"',
        "subsystem": "lan",
        "sw": device_2["mac"],
...
async def test_remove_clients(hass, aioclient_mock, mock_unifi_websocket):
    ...
    mock_unifi_websocket(data={
        "meta": {
            "message": MESSAGE_CLIENT_REMOVED
        },
        "data": [client_1],
    })
    await hass.async_block_till_done()
...
async def test_option_ssid_filter(hass, aioclient_mock, mock_unifi_websocket):
    ...
    assert hass.states.get("device_tracker.client").state == STATE_HOME
    assert hass.states.get(
        "device_tracker.client_on_ssid2").state == STATE_HOME
